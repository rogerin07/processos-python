...
class ProcessSchedulerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Escalonador de Processos")
        self.process_list = []
    ...
    def setup_ui(self):
        ...
        ttk.Button(frame, text="Adicionar Processo", command=self.add_process).grid(row=0, column=4, padx=5, pady=5)
        ...
        self.process_table = ttk.Treeview(frame, columns=("ID", "Tempo de Execução"), show="headings")
        self.process_table.heading("ID", text="ID")
        self.process_table.heading("Tempo de Execução", text="Tempo de Execução")
        self.process_table.grid(row=2, column=0, columnspan=5, padx=5, pady=5)
        
        ttk.Button(frame, text="Executar Escalonamento", command=self.run_scheduler).grid(row=3, column=0, columnspan=5, pady=10)
        ttk.Button(frame, text="Resetar", command=self.reset).grid(row=3, column=5, padx=5, pady=10)
        
        self.result_label = tk.Label(frame, text="", fg="blue", font=("Arial", 10, "bold"))
        self.result_label.grid(row=4, column=0, columnspan=5, pady=5)
    ...
    def add_process(self):
        ...
        if not process_id or not burst_time.isdigit():
            messagebox.showerror("Erro", "ID do processo ou tempo de execução inválido!")
            return
        
        self.process_list.append({"id": process_id, "burst_time": int(burst_time)})
        self.process_table.insert("", "end", values=(process_id, burst_time))
        self.process_id_entry.delete(0, tk.END)
        self.burst_time_entry.delete(0, tk.END)
